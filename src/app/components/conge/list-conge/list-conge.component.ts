...
saveData(): void {
  ...
  const data = {
    ...
    date_retour:this.dataSend.date_retour,
...
retrievelist():void {
  ...
  showItem(id: any): void {
    this.reset();
    this.congeService.get(id).subscribe(
      data => {
  ...
  {
    $.toast().reset('all');
    this.congeService.update(id,data)
    .subscribe(
      response => {
        this.refreshList();
        this.newData();
  ...
  saveDelete(id:any):void{
    this.congeService.delete(id.id).subscribe(
      data=>{
